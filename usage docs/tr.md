...
Bu döküman, **CodinLab** projesine içerik eklemeye yardımcı olması için hazırlanmıştır.

Proje içerisinde default olarak dil dosyası oluşturulmuştur. Bu dosyasının içerisinde mevcut diller (go,javascript,python,c++) yer almaktadır. Ekleyeceğiniz içeriğin dili mevcut değil ise ilk önce o dili eklemeniz gerekmektedir. Eğer içerik ekleyeceğiniz dil mevcut ise bu adımı geçebilirsiniz. 

Dillere içerik olarak iki ayrı kategorimiz bulunmaktadır ;
...
Proje içerisinde dil dosyası object klasörü altında yer almaktadır. Buradaki `inventory.json` dosyası içerisinde default ekli diller bulunmaktadır. Yeni bir dil eklenmek istendiği zaman bu dosya içerisine ekleme yapılması gerekmektedir.
...
- **`languages`**: Json dosyası içerisine yazmanız gereken son özellik ise türkçe ve ingilizce olacak şekilde; eklenecek programlama dilinin türkçe/ingilizce olarak ayrı ayrı başlık ve açıklamasıdır.
...
2. Bu eklenen lab için 'code templates' yazınız. İleri de detaylı açıklanacaktır.
    ...
    "languages": [
        {
            ...
            "hint":"Hint for solving the lab" //yalnızca lablarda eklenir path de yoktur
        },
        {
            "lang": "tr",
...
- `$out$` değeri `quest.json` dosyasında yer alan gerekli test'in output değerini ifade eder.
...
Fonksiyona gerekli input verilerek, fonksiyondan dönen sonuç bir değişkene atılır ve kullanıcının yazdığı kodun dönüşü , beklenen dönüş'e eşit mi diye bakılır. Eğer eşit değilse gerekli format ekrana yazdırılır ve return edilerek durdurulması sağlanır.
