                ...
                _titles?.length > 1
                    ? _titles.map((item, index) => (
                        (index == _titles.length - 1)
                            ? <Typography key={index} variant='caption' sx={{ display: 'flex', alignItems: 'center', gap: '8px', cursor: 'default', color: theme => `${theme.palette.primary.main} !important` }}>{item.icon}{item.title}</Typography>
                            : !item?.path
