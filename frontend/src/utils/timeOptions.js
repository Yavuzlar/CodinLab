...
export const howLongAgo = ({ date, short }) => {
  if (dateControl(date)) {
    ...
    if (years > 0)
      return short
        ? `${years}y`
        : `${years == 1 ? `${years} year` : `${years} years`} ago`;
    else if (months > 0)
      return short
        ? `${months}mo`
        : `${months == 1 ? `${months} month` : `${months} months`} ago`;
    else if (days > 0)
      return short
        ? `${days}d`
        : `${days == 1 ? `${days} day` : `${days} days`} ago`;
    else if (hours > 0)
      return short
        ? `${hours}h`
        : `${hours == 1 ? `${hours} hour` : `${hours} hours`} ago`;
    else if (minutes > 0)
      return short
        ? `${minutes}m`
        : `${minutes == 1 ? `${minutes} minute` : `${minutes} minutes`} ago`;
    else if (seconds > 0)
      return short
        ? `${seconds}s`
        : `${seconds == 1 ? `${seconds} second` : `${seconds} seconds`} ago`;
    else return "şimdi";
  } else return null;
};
