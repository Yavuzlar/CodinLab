...
func (h *PrivateHandler) AnswerLab(c *fiber.Ctx) error {
	...
	tmpContent, err := h.services.CodeService.CodeDockerTemplateGenerator(codeTemplate.GetTemplatePath(), lab.GetQuest().GetFuncName(), answerLabDTO.UserCode, inventoryInformation.GetName(), lab.GetQuest().GetTests())
	if err != nil {
		return err
	}
